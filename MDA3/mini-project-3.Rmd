...
vancouver_trees_year%>%
  filter(year_planted == 1998)%>%
  group_by(genus_name)%>%
  summarise(n = n())%>%
  drop_na()%>%
  ggplot(aes(genus_name,n)) +
  geom_point() +
...
vancouver_trees_month$month_name <- month.name[vancouver_trees_month$month_planted]
...
vty_acer <- vancouver_trees_year%>%
  ...
  filter(genus_name == "ACER")%>% #keep only ACER trees
  drop_na()%>% #drop any NA
  summarise(mean_diameter = mean(diameter))#calculating mean diameter for each year
#print(vty_acer)

vty_acer$year_planted <- as.numeric(vty_acer$year_planted)

model_vty_acer <- lm(mean_diameter ~ I(year_planted-1989), vty_acer)
...
```{r}
prediction_vty_acer <- broom::augment(model_vty_acer) #use the augment function of broom package
broom::
print(prediction_vty_acer)
```
The fitted column correspond to the prediction of the diameter for each year depending on the model obtained before. 
<!----------------------------------------------------------------------------->
...
write_csv(finalversion_vancouver_trees, here::here("output/finalversion_vancouver_trees.csv"))
```
<!----------------------------------------------------------------------------->
